...
def build_missing_date_list(precipitation_output_directory: Path, start_date: datetime, end_date: datetime):
    ...
    for file in file_list:
        ...
        if len(missing_data_list):
            ...
            if len(new_missing_times):
                missing_data_list.extend(pd.to_datetime(new_missing_times).to_pydatetime().tolist())
...
def main():
    ...
    parser.add_option("--LastNHours", dest="last_n_hours",
                      help="Number of hours of XMRG files to download.", default=None,
                      type="int")
    ...
    parser.add_option("--DownloadDirectory", dest="download_directory",
                      help="Directory to download XMRG files to.", default=None)

    parser.add_option("--ConfigurationFile", dest="config_file",
                      help="Configuration Settings.", default=None)

    parser.add_option("--FillGaps", dest="fill_gaps", action="store_true", default=False,
    ...
    delete_compressed_source_file = config_file.get("xmrg", "delete_compressed_source_file")
    ...
    precipitation_temp_output_directory = Path(config_file.get("xmrg", "precipitation_temp_output_directory"))
    precipitation_temp_output_directory.mkdir(exist_ok=True)
    precipitation_output_directory = Path(config_file.get("xmrg", "precipitation_output_directory"))
    precipitation_output_directory.mkdir(exist_ok=True)
    #data_saver = nexrad_xenia_sqlite_saver(sqlite_file)
    data_saver = nexrad_csv_saver(precipitation_output_directory,
    ...
    wkt_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), "boundaries.csv")
    ...
    missing_dates = build_missing_date_list(precipitation_temp_output_directory, utc_start_date, utc_end_date)

    task_id = uuid4()
    if len(missing_dates):
        xmrg_iterator = xmrg_file_iterator(date_list=missing_dates,
