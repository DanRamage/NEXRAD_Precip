    ...
    def save(self, xmrg_results_data: xmrg_results):
        ...
        for boundary_name, boundary_results in xmrg_results_data.get_boundary_data():
            if boundary_name not in self._precip_files:
                ...
                file_path = (
                    self._temp_directory / f"{cleaned_boundary_name}_unsorted.csv"
                )
                write_header = False
            ...
            try:
                ...
                utc_datetime = self._src_tz.localize(
                    datetime.strptime(xmrg_results_data.datetime, "%Y-%m-%dT%H:%M:%S")
                )
                # local_datetime = utc_datetime.astimezone(self._dst_tz).strftime("%Y-%m-%dT%H:%M:%S")
                precip_file_obj.write(
                    f"{boundary_name},{utc_datetime},{utc_datetime},{avg:0.6f}\n"
                )
    ...
    def finalize(self):
        ...
        try:
            for boundary_name, precip_file_obj in self._precip_files.items():
                ...
                self._logger.info(
                    f"Sorting file file: {unsorted_filename} into file: {filename}"
                )
                final_filename = Path(directory) / filename

                unsorted_pd_df = read_csv(
                    unsorted_filename,
                    dtype={
                        "Area": str,
                        "Start Time": str,
